...
- **分享给别人的机器若没有 WorkBuddy**：需对方 `npm i -g @tencent-ai/codebuddy-code` 并确保 `codebuddy` 在 `PATH`，或在 `.env` 设 `CODEBUDDY_CLI_PATH` 指向 CLI。否则 `query()` 拉不起进程 → 只能演示模式或连不上。
...
**两种模式**
- 演示模式：未填 `CODEBUDDY_API_KEY`，后端返回模拟流式回复，验收交互闭环用，无需账号。
- 真实模式：填了 Key，真正调用 CodeBuddy AI。
...
| 仍是演示回复 | 检查 `.env` 的 `CODEBUDDY_API_KEY` 是否填写并重启后端 |
